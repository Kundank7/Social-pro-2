...
import { Search, AlertCircle, Loader2, CheckCircle, Clock, RefreshCw, Package } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from "@/components/ui/card"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert"
import { getOrderById, getOrderByEmail } from "@/lib/actions"
...
export default function TrackOrderPage() {
  ...
  const statusConfig = {
    Pending: { color: "text-yellow-500 bg-yellow-100 dark:bg-yellow-900/30", icon: Clock },
    "In Review": { color: "text-blue-500 bg-blue-100 dark:bg-blue-900/30", icon: RefreshCw },
    Processing: { color: "text-purple-500 bg-purple-100 dark:bg-purple-900/30", icon: Package },
    Completed: { color: "text-green-500 bg-green-100 dark:bg-green-900/30", icon: CheckCircle },
  }
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (searchBy === "orderId") {
        ...
        result = await getOrderByEmail(email)
      }
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-start">
                      <div
                        className={`mt-1 w-4 h-4 rounded-full ${
                          order.status === "In Review" || order.status === "Processing" || order.status === "Completed"
                            ? "bg-green-500"
                            : "bg-gray-300"
                        } mr-3`}
                      ></div>
                    ...
                    <div className="flex items-start">
                      <div
                        className={`mt-1 w-4 h-4 rounded-full ${
                          order.status === "Processing" || order.status === "Completed" ? "bg-green-500" : "bg-gray-300"
                        } mr-3`}
                      ></div>
                      <div>
